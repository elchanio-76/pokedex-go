...
- [x] 3. Modify `commandExplore` for scoped validation
  - [x] 3.1 Rewrite `commandExplore` in `main.go` for scoped behavior
    - If `cfg.CurrentLocation == nil`: print "No location set. Use 'visit <name>' to visit a location first", return nil
  ...
  - [x] 3.2 Write property test for area validation rejection
  ...
  - [x] 3.3 Write property test for successful explore updating state
  ...
  - [x] 3.4 Write property test for failed explore preserving state
